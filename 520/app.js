...
$(function () {
    ...
    k.x = 600, k.y = 80, f.addChild(k), d.addChild(f), createjs.Tween.get(h).wait(2e3).to({alpha: 0}, 500).to({text: 6}).wait(600).to({alpha: 1}, 500).wait(500).call(ib).to({
        ...
        scaleY: 1,
        alpha: 1
    }, 500).wait(1300).to({alpha: 0}), createjs.Tween.get(j).wait(14e3).to({alpha: .6}, 100, createjs.Ease.quintIn).to({
        x: 579,
        y: 80,
        scaleX: .18,
        scaleY: .18,
    ...
    d.addChild(s);
    for (var C = s.getNumChildren(), D = r + 180 * C, z = 0; C > z; z++) createjs.Tween.get(s.getChildAt(z)).wait(r + 180 * z).to({
        scaleX: 1,
        scaleY: 1,
        x: 0,
        y: 0
    }, 180, createjs.Ease.elasticOut);
    var E, F, G, H = Math.PI * 2 / 50, I = Math.PI, J = new createjs.Container, K = new Image;
    K.src = "http://xiemin.me/love/sparkle_21x23.png";
    var L = {images: [K], frames: {width: 21, height: 23, regX: 10, regY: 11}};
    E = new createjs.BitmapAnimation(new createjs.SpriteSheet(L));
    for (var z = 0; 50 > z; z++) I += H, F = 435 + 5 * 16 * Math.pow(Math.sin(I), 3), G = 260 - 5 * (13 * Math.cos(I) - 5 * Math.cos(2 * I) - 2 * Math.cos(3 * I) - Math.cos(4 * I)), M(F, G);
    d.addChild(J);
    for (var N = J.getNumChildren(), O = D + 80 * N, z = 0; N > z; z++) createjs.Tween.get(J.getChildAt(z)).wait(D + 80 * z).to({alpha: Math.random() * .5 + .2}, 80, createjs.Ease.elasticOut);
    ...
    d.addChild(P);
    for (var W = P.getNumChildren(), X = O + 3e3, z = 0; W > z; z++) createjs.Tween.get(P.getChildAt(z)).wait(O).to({
        scaleX: 1,
        scaleY: 1,
        x: 0,
        y: 0
    }, 3e3, createjs.Ease.circOut);
...
Math.random = function (a, b, c) {
    if (arguments.length === 0) return native_random();
